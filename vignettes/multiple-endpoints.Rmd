...
v1s <- vpc(fit.TOS, show=list(obs_dv=T), scales="free_y") +
    ylab("Warfarin Cp [mg/L] or PCA") +
    xlab("Time [h]");
v2s <- vpc(fit.TOS, show=list(obs_dv=T), pred_corr = TRUE) +
...
v1f <- vpc(fit.TOF, show=list(obs_dv=T), scales="free_y") +
    ylab("Warfarin Cp [mg/L] or PCA") +
    xlab("Time [h]");
v2f <- vpc(fit.TOF, show=list(obs_dv=T), pred_corr = TRUE) +
        ylab("Prediction Corrected Warfarin Cp [mg/L] or PCA") +
    xlab("Time [h]")
